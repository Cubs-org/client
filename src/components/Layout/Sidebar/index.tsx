...
export const Sidebar = ({ layout, handleSetLayout }: ISidebar) => {
    ...
    const userName = (user?.data?.name as string) || 'Usuário',
        userIcon =
            (user?.data?.icon as string) || '/src/assets/default-user.jpg'

    const { pathname } = useLocation()

    const [sidebarVisibility, setSidebarVisibility] = useState(false)

    const menu_option_default =
        'w-full lg:w-[60px] lg:h-[60px] px-3 py-2 font-bold text-dark-600 rounded-md cursor-pointer flex justify-start items-center lg:justify-center gap-3 hover:text-light-300 hover:bg-violet-500 hover:shadow-menu dark:text-light-600'
    ...
    return (
        <>
            ...
            <div
                className={clsx(
                    'absolute lg:relative w-full min-h-screen lg:w-[80px] lg:min-w-[80px] lg:min-h-[95vh] lg:max-h-[90vh] flex z-40 flex-col justify-evenly lg:justify-between items-center gap-0 lg:gap-3 lg:rounded-2xl px-3 py-2 bg-light-100 dark:bg-dark-900 transition-all border border-light-700 dark:lg:border-dark-700',
                    {
                        // "lg:absolute lg:-translate-x-[80px]" : !layout,
                        'lg:-ml-[80px]': !layout,
                        'lg:translate-x-0 -translate-x-[100vw]':
                            !sidebarVisibility,
            ...
            >
                ...
                <div className="w-full flex flex-col items-center gap-2">
                    <div className="w-3/5 mb-8 lg:mb-3 lg:w-[60px] lg:h-[60px] lg:mt-4 flex items-center flex-row gap-3 text-violet-500">
                        <Logo
                            size={32}
                            className="lg:min-w-full fill-violet-500"
                        />
                        <span className="block lg:hidden text-3xl font-extrabold text-primary">
                            Cub's
                        </span>
                    </div>
                    ...
                    <div className="w-3/5 h-full lg:w-full flex flex-col lg:items-center gap-2">
                        {pages.map((page, index) => (
                            ...
                            >
                                <a href={page.link}>
                                    <div
                                        className={clsx(menu_option_default, {
                                            '!text-violet-500 hover:!text-light-300 w-full':
                                                page.link == pathname,
                                        })}
                                    >
                                        {page.icon}
