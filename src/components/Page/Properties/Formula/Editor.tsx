...
export const Editor = ({ content }) => {
    const [code, setCode] = useState(content || '// Vazio...')

    const clickable = ['Option 1', 'Option 2', 'Option 3']
    ...
    return (
        <div className="p-2 grid grid-col-2 gap-2">
            ...
            <div className="col-span-2 flex gap-1 items-center justify-between p-2 rounded-md bg-glass-light dark:bg-glass-dark">
                <b>Resultado:</b>
                {(() => {
                    try {
                        return (
                            <span className="text-normal text-violet-500 font-semibold">
                                {eval(preview(code))}
                            </span>
                        )
                    } catch (error) {
                        // Tratar o erro aqui
                        console.error(
                            'Erro durante a avaliação do código:',
                            error
                        )
                        return (
