...
const Tag = ({ value, color, onDelete }: ITag) => {
  ...
  return (
  <div className={clsx("py-0.5 rounded-md flex items-center pl-1 pr-1.5 gap-x-1", {
    ...tagColor
  })}>
    <CgClose 
      size={16}
      onClick={onDelete} 
      className={clsx("hover:bg-light-400 dark:hover:bg-dark-500 rounded-full p-0.5 cursor-pointer", {
        ...tagColor 
      })} 
    />
    {value}
...
export const Tags = ({ data, dictionary, onClick, onDelete, allowMultiPaste = true }: ITags) => {
  ...
  const serializePaste = (str: string) => {
    const possibleTags = str.split(/[,;.\s]+/).filter(item => item.trim() !== '');
    const verifiedTags = possibleTags.filter(tag =>
      dictionary.some(item => item.name.toLowerCase() === tag.toLowerCase())
    );
    return verifiedTags;
  };
  ...
  return (
    <div className="relative border border-light-500 dark:border-dark-500 rounded-md">
      <ul className="flex flex-wrap p-1.5 gap-1">
        ...
        <li className="grow">
          <TextArea value={search} handle={handleAdd} updateOnChange={true} outlineDisabled />
        </li>
      </ul>
      {!onPaste && (
        <div className={clsx("absolute bottom-0 left-0 w-full translate-y-full z-10", {
          "hidden": !search,
          "block": search
        })}>
          <Discover dictionary={dictionary} searchableBy={{ name: search }} onClick={handleClick} />
        </div>
      )}
    </div>
  )
