...
export const NewItem = () => {
    ...
    return (
            ...
            content={
                <div className="flex flex-col gap-1 p-1 min-w-[200px]">
                    {options.map((opt, _i) => (
                        <div key={`${_i}-${opt}`}>
                            <Button
                                classNames="w-full bg-transparent justify-start text-dark-300 dark:text-light-300 hover:text-light-300 hover:bg-violet-500"
                                onClick={opt.onClick}
                            >
                                {opt.icon}
                                {opt.label}
        ...
        >
            <span className="flex p-1 md:p-2 md:pr-3 gap-1 items-center bg-violet-500 hover:bg-violet-600 text-light-100 text-base font-semibold rounded-md">
                <BsPlus size={24} />
