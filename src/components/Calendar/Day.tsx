    ...
    return (
        ...
        >
            {items
                .sort((a, b) => {
                    const hierarchyA =
                        a.timeline.find((timeline) => timeline.day === day)
                            ?.hierarchy || 0
                    const hierarchyB =
                        b.timeline.find((timeline) => timeline.day === day)
                            ?.hierarchy || 0
                ...
                .map((item, _i) => {
                    if (!isPage) {
                        ...
                        const start = item?.properties
                            ?.find((p: any) => p.type === 'datetime')
                            .data.start.split(' ')[0]
                        const end = item?.properties
                            ?.find((p: any) => p.type === 'datetime')
                            .data.end.split(' ')[0]
            ...
            {items.some((item) =>
                item?.timeline.some(
                    (timeline) =>
                        timeline.day === day && timeline.hierarchy >= 3
                )
            ) &&
                !isPage && (
                    <div
            ...
            <span
                className={clsx(
                    'w-6 h-6 text-xs font-medium grid place-items-center rounded-md',
                    {
                        'bg-violet-500 text-light-100 font-semibold':
                            today === day,
                    }
                )}
            >
