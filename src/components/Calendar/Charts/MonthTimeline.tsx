...
export const Timeline = ({
    ...
    const isFiltered = ['task', 'event', 'reminder'].includes(
        timelineProps?.title
    )
    ...
    let style: CSSProperties = {
        width:
            item && width
                ? `calc(${width * 100}% + ${range && range * 5 - 5 + 'px'})`
                : '100%',
    }
    ...
    return (
        <div
            style={style}
            className={clsx(
                'absolute z-10 left-0 min-w-full min-h-[15px] rounded-md shadow-none md:shadow-md text-black flex items-center px-2 transition-all duration-[.5s]',
                {
                    'top-0': hierarchy === 1,
                    ...
                    '!bg-violet-500': timelineProps?.data?.color === 'purple',
        ...
        >
            ...
            {isFiltered && (
                ...
                >
                    {item.title || 'Sem título'} @ {Math.floor(rangeDays)}d
                </span>
            )}
        </div>
    )
