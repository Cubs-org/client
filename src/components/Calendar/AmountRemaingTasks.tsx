    ...
    <div className="w-[600px] flex flex-col gap-2">
        {items.map((item, _i) => (
            ...
            >
                ...
                <div className="min-w-[20ch] text-dark-100 dark:text-light-400 tracking-wide text-xs font-semibold ">{formatDate(item.startDate)} até {formatDate(item.endDate)}</div>
            </div>
        ))}
    </div>
)