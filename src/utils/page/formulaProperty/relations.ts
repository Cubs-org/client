...
const getPropertyValue = (rowPage: PageProps, relationName) => {
    ...
    if (relationName === 'Título') relationValue = rowPage.title
    else if (relationName === 'Criado em') relationValue = rowPage.createdAt
    else if (relationName === 'Atualizado em') relationValue = rowPage.updatedAt
    else if (relationName === 'Deletado') relationValue = rowPage.trash
    else {
        if (rowPage.properties) {
            let property = rowPage.properties.find(
                (property) =>
                    String(property.title).toLowerCase() ===
                    String(relationName).toLowerCase()
            )
            ...
            switch (property?.type) {
                ...
                case 'formula':
                    relationValue = eval(
                        (property?.data?.value as string) || 'Vazio'
                    )
                    break
                case 'datetime':
                    if (property?.data?.start && property?.data?.end) {
                        const start = formatDateToPreview(
                                property?.data?.start ||
                                    new Date().toISOString()
                            ),
                            end = formatDateToPreview(
                                property?.data?.end || new Date().toISOString()
                            )
...
export const getPropRelationFromFormula = (
    rowPage: PageProps,
    formula: string
): string => {
